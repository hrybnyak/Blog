...
namespace Blog.Controllers
{
    ...
    public class CommentController : ControllerBase
    {
        ...
        public IActionResult UpdateComment([FromBody] CommentDTO comment)
        {
            try
            {
                _commentService.UpdateComment(comment, AuthInfo());
        ...
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public IActionResult DeleteComment([FromBody] CommentDTO comment)
        {
            try
            {
                _commentService.DeleteComment(comment, AuthInfo());
                return NoContent();
            }
            catch (ArgumentNullException ex)
            {
