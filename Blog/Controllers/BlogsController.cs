...
namespace Blog.Controllers
{
    ...
    public class BlogsController : ControllerBase
    {
        ...
        public BlogsController(IBlogService accountService, ILogger<BlogsController> logger)
        {
            _logger = logger;
        ...
        public IActionResult DeleteBlog(int id)
        {
            try
            {
                _blogService.DeleteBlog(id, AuthInfo());
                 return NoContent();
            }
            catch (ArgumentNullException ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [Authorize(Roles = "RegularUser")]
        public IActionResult UpdateBlogName(int id,[FromBody] BlogDTO blog)
        {
            try
            {
                _blogService.UpdateBlogName(id, blog, AuthInfo());
                return NoContent();
            }
            catch (ArgumentNullException ex)
            {
